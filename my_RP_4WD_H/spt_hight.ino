void fw_ch_r(int sp, volatile int encoderPoss_fw)
  {
    ...
    while(encoderPoss < encoderPoss_fw)      
        {                 
          ...
          PID_output = (4 * P) + (0.00015 * I) +(5* D); 
          
          Motors(sp - PID_output, sp + PID_output, sp - PID_output, sp + PID_output); 
          if(digitalRead(26)==0)
            {
              chopsticks = 1;
         ...
         if(mcp_f(0) < md_mcp_f(0) || mcp_f(1) < md_mcp_f(1))  //------->> ตวจสอบเส้นด้านหน้า
            {                 
               ...
               Motor(-100,-100,-100,-100);delay(70); 
                  ...
                  {                  
                    while(1)
                      {                        
                        ...
                        if( mcp_f(0) < md_mcp_f(0))
                          {
                             Motor(-100,-100,-100,-100);delay(20);
                             Motor(0,0,0,0);delay(300);
                             check_color_roll_bz(); 
    ...
    while(encoderPoss > -255)           
        {          
          ...
          Motor(62-PID_output, -(62 - PID_output),-(62 + PID_output), 62 + PID_output); 
         // Motor(61, -61,-61, 61); 
          if(mcp_f(1)<md_mcp_f(1) || mcp_f(3)<md_mcp_f(3)) //----------------->> ถ้ามีเส้นดำด้านข้าง
            {
              ...
              if(mcp_f(1)<md_mcp_f(1)&& mcp_f(4)>md_mcp_f(4))
                ...
                {
                  do{Motor(60,-60,-80,-40);}while(mcp_f(4)>md_mcp_f(4));
                  Motor(0,0,0,0);delay(300);               
                }
              else
                {                   
