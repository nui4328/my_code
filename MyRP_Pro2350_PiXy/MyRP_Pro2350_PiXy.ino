...
void setup() {
 ...
 // arm_down_open();
  ...
  ///-------------------------------------------------------------------->>>>
    ...
    arm_down_close();
    fline(10, 10, 0.35, 0, 'f', 's', 100, "a1", 20); 
    get_color_pixy();
    delay(500);
    arm_ready();
    fline(10, 10, 0.35, 10, 'n', 's', 100, "a1", 20); 
    arm_down_close();
    delay(500);
    bline(40, 40, 0.35, 0, 'c', 'r', 50, "a5", 40);
    fline(10, 10, 0.35, 0, 'f', 's', 100, "a1", 20);
    place_left_in(20, 30, 20) ;
    pixy_vs_tcs();
...
void loop() {
    searchColor() ;
  //float color[] = {my_tcs('r'), my_tcs('g'), my_tcs('b') };
 // check_floor_M();
